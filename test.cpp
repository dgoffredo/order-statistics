...
void test_tree() {
    ...
    // `percentile`
    // TODO: actually make it a test
    const auto by_age = [](const Fish& fish) { return fish.age; };
    order_statistics::Tree<Fish, decltype(by_age)> fish_tree;
    for (const Fish& fish : fishes) {
        std::cerr << "inserting " << fish << '\n';
        fish_tree.insert(fish);
        std::cerr << "size: " << fish_tree.size() << "\t age:" 
            << " p1=" << fish_tree.percentile(1)[0].age
            << " p10=" << fish_tree.percentile(10)[0].age
            << " p50=" << fish_tree.percentile(50)[0].age
            << " p90=" << fish_tree.percentile(90)[0].age
            << " p99=" << fish_tree.percentile(99)[0].age
